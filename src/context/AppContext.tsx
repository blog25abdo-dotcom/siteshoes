...
import { Product, Category, ContactMessage, User, CartItem } from '../types';
...
export const AppProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      try {
        ...
        const savedProducts = localStorage.getItem('ramaFashion_products');
        const savedCategories = localStorage.getItem('ramaFashion_categories');
        const savedMessages = localStorage.getItem('ramaFashion_messages');
  ...
  const addToCart = (product: Product, selectedSize?: string, quantity: number = 1) => {
    ...
    // Sauvegarder dans localStorage
    setTimeout(() => {
      const currentCart = JSON.parse(localStorage.getItem('shoesParadise_cart') || '[]');
      const newCart = [...currentCart, cartItem];
      localStorage.setItem('shoesParadise_cart', JSON.stringify(newCart));
    }, 100);
  };
  ...
  const clearCart = () => {
    ...
    localStorage.removeItem('shoesParadise_cart');
  };
  ...
  useEffect(() => {
    const savedCart = localStorage.getItem('shoesParadise_cart');
    if (savedCart) {
      try {
