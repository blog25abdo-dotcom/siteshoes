        ...
        <div className="flex items-center justify-between py-4">
          ...
          <Link to="/" className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 -mt-1">Style & Confort</p>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-8">
            ...
            >
              Accueil
              {isActive('/') && (
                <div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-to-r from-teal-600 to-cyan-600"></div>
              )}
            </Link>
            <Link
              to="/products"
            ...
            >
              Chaussures
              {isActive('/products') && (
                <div className="absolute -bottom-1 left-0 right-0 h-0.5 bg-gradient-to-r from-orange-600 to-red-600"></div>
              )}
            </Link>
            
            {/* Categories Dropdown */}
            <div className="relative group">
              <button className="font-medium text-gray-700 hover:text-orange-600 transition-colors flex items-center">
                Catégories
          ...
          <div className="hidden md:flex items-center flex-1 max-w-md mx-8">
            <form onSubmit={handleSearch} className="relative w-full">
              <input
                ...
                className="w-full px-4 py-2 pl-10 pr-4 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-transparent bg-gray-50 focus:bg-white transition-colors"
              />
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-4 h-4 text-gray-400" />
              <button
                type="submit"
                className="absolute right-1 top-1/2 transform -translate-y-1/2 bg-orange-600 text-white px-4 py-1 rounded-full hover:bg-orange-700 transition-colors text-sm"
              >
                Rechercher
              </button>
            </form>
                      ...
                      <>
                          ...
                          {state.cart.map(item => (
                            <div key={item.id} className="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                              ...
                              <div className="flex-1 min-w-0">
                                ...
                                {item.selectedSize && (
                                  <p className="text-xs text-gray-600 truncate">Taille: {item.selectedSize}</p>
                                )}
                                <p className="text-sm font-bold text-orange-600 truncate">
                                  {item.product.price} DH x {item.quantity}
                                </p>
                              </div>
                            </div>
                          ))}
                        ...
                        <div className="border-t border-gray-200 pt-3 mt-3">
                          ...
                          <Link
                            ...
                            className="w-full bg-gradient-to-r from-orange-600 to-red-600 text-white py-3 rounded-lg hover:from-orange-700 hover:to-red-700 transition-colors text-center block font-medium text-sm sm:text-base"
                          >
                            Voir le panier
                          </Link>
                        </div>
        ...
        <div className="md:hidden pb-4">
          <form onSubmit={handleSearch} className="relative">
            <input
              ...
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Rechercher un produit..."
              className="w-full px-4 py-3 pl-10 pr-20 border border-gray-300 rounded-full focus:outline-none focus:ring-2 focus:ring-orange-300 focus:border-transparent bg-gray-50 focus:bg-white transition-colors"
            />
            <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-400" />
            <button
              type="submit"
              className="absolute right-2 top-1/2 transform -translate-y-1/2 bg-orange-600 text-white px-4 py-1.5 rounded-full hover:bg-orange-700 transition-colors text-sm"
            >
              Rechercher
            </button>
          </form>
            ...
            <div className="space-y-2">
              ...
              {state.categories.map(category => (
                <Link
                  ...
                  to={`/products?category=${category.id}`}
                  onClick={() => setIsMenuOpen(false)}
                  className="block py-2 px-8 text-gray-600 hover:text-orange-600 hover:bg-orange-50 rounded-lg transition-colors"
                >
                  {category.name}
                </Link>
              ))}
